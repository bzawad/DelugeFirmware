...
- `Visualizer (VISU)` <Badge text="c1.4" variant="tip" />
  - When On, enables real-time audio visualizations on the OLED display. Provides four visualization modes: Waveform, Spectrum, Equalizer, and Circle to help monitor audio output and frequency content. Works by default with the VU meter or can be toggled independently.
