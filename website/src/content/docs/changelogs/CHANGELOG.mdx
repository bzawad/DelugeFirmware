...
- Four visualization modes: Waveform (time-domain), Spectrum (frequency-domain using FFT), Equalizer (16-band frequency analysis), and Circle (concentric frequency bands)
...
- Hot keys allow switching between modes: :key[SYNTH] for Waveform, :key[KIT] for Spectrum, :key[MIDI] for Equalizer, :key[CV] for Circle
