...
void renderVisualizerCube(oled_canvas::Canvas& canvas) {
	...
	// Scale so 120 BPM gives moderate rotation, 240 BPM gives fast rotation
	float rotation_speed = (tempo_bpm / 120.0f) * kBaseRotationSpeed;
	...
	frame_counter++;
	float time_seconds = static_cast<float>(frame_counter) / 30.0f; // 30 FPS
...
void renderCube(oled_canvas::Canvas& canvas, float rotation_angle, float scale, int32_t center_x, int32_t center_y,
	...
	float tilt_angle = kCubeTiltAngleDegrees * M_PI / 180.0f; // Convert degrees to radians
	...
	float cos_x = std::cos(rotation_angle * kRotationMultiplierX + kRotationOffsetXDegrees * M_PI / 180.0f);
	float sin_x = std::sin(rotation_angle * kRotationMultiplierX + kRotationOffsetXDegrees * M_PI / 180.0f);
	...
	for (int32_t i = 0; i < kCubeEdges; i++) {
		...
		float fovy = kFieldOfViewDegrees * M_PI / 180.0f; // Convert degrees to radians
