...
FFTResult computeVisualizerFFT() {
	...
	for (int32_t i = 0; i < kSpectrumFFTSize; i++) {
		...
		// Apply Hanning window: multiply Q15 sample by Q31 window, shift right by 31 to get Q15 result
		// Use 64-bit intermediate to prevent overflow
		int64_t windowed_sample =
		    (static_cast<int64_t>(sample) * static_cast<int64_t>(spectrum_hanning_window[i])) >> 31;
