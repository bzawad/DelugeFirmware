...
// Replaces the missing drawLine method for OLED canvas 1.2 compatibility
void drawLine(deluge::hid::display::oled_canvas::Canvas& canvas, int32_t x1, int32_t y1, int32_t x2, int32_t y2) {
	...
	if (y1 == y2) {
		if (x1 <= x2) {
			canvas.drawHorizontalLine(y1, x1, x2);
		}
		else {
			canvas.drawHorizontalLine(y1, x2, x1);
		}
		return;
	}
	...
	if (x1 == x2) {
		if (y1 <= y2) {
			canvas.drawVerticalLine(x1, y1, y2);
		}
		else {
			canvas.drawVerticalLine(x1, y2, y1);
		}
		return;
	}
