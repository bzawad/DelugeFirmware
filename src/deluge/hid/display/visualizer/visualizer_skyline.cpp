...
// 16-band equalizer center frequencies (copied from visualizer_bar_spectrum.cpp)
...
void updateSkylineBands() {
	...
	for (int i = 0; i < kSkylineNumBands; i++) {
		...
		display_value = clamp01(display_value);
		...
		g_skylineBandValues[i] = clamp01(g_skylineBandSmoothed[i]);
	}
}
...
void drawSun(oled_canvas::Canvas& canvas, int32_t min_x, int32_t max_x, int32_t region_top, int32_t region_bottom,
	...
	// Use fixed radius for consistent sun appearance
...
void renderVisualizerSkylineInternal(oled_canvas::Canvas& canvas) {
	...
	if (!g_initialized) {
		...
		g_gridOffset = 0.0f;
		g_initialized = true;
		for (auto& val : g_skylineBandValues) {
			val = 0.0f;
		}
		for (auto& val : g_skylineBandSmoothed) {
			val = 0.0f;
		}
	}
