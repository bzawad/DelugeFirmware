...
static uint32_t last_audio_time = 0; // Last time we had audio (for persistence)
...
void renderVisualizerStarfield(oled_canvas::Canvas& canvas) {
	...
	if (sample_magnitude < kSilenceThreshold) {
		...
		if (is_silent) {
			...
			// Audio detected, update last_audio_time
	...
	smoothed_amplitude = smoothed_amplitude * kSmoothingAlpha + current_amp * kSmoothingBeta;

	// Optional (safe on monochrome): use amplitude to slightly increase speed
	float audio_speed_boost = smoothed_amplitude * 0.01f;
	...
	float base_speed = kBaseSpeed * (bpm / 120.0f);
