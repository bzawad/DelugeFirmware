...
void renderVisualizerWaveform(oled_canvas::Canvas& canvas) {
	...
	for (uint32_t i = 0; i < num_samples_to_display; i++) {
		...
		if (!is_first_point && last_x >= 0 && last_x != x) {
			...
			int32_t dx = abs(x - last_x);
			int32_t dy = abs(y - last_y);
			int32_t sx = (last_x < x) ? 1 : -1;
			int32_t sy = (last_y < y) ? 1 : -1;
			int32_t err = dx - dy;

			int32_t current_x = last_x;
			int32_t current_y = last_y;

			while (true) {
				canvas.drawPixel(current_x, current_y);
			...
			// First point of this frame or can't draw line, just draw a pixel
