...
void renderVisualizerPulseGrid(oled_canvas::Canvas& canvas) {
	...
	// Check for silence by examining representative bins OR low amplitude
	// If all bins are very small OR amplitude is below threshold, don't update display to avoid flicker
	if (fft_result.isSilent && !has_audio) {
		return;
	}
